...
import { customersTourSteps } from '../components/tours/customersTour';
...
export default function Customers() {
  ...
  const customerStats = React.useMemo(() => {
    ...
    orders.forEach((o) => {
      ...
      const matchedCust = customers.find((c) => {
        const nameA = String(c.name || '')
          .trim()
          .toLowerCase();
        const nameB = String(cName || '')
          .trim()
          .toLowerCase();
      ...
      if (dDate) {
        const d = new Date(
          typeof dDate === 'object' && dDate.seconds ? dDate.seconds * 1000 : dDate
        );
        if (!isNaN(d.getTime())) {
  ...
  const enriched = customers.map((c) => {
    ...
    const isChurning =
      lastOrderDate && new Date() - new Date(lastOrderDate) > 60 * 24 * 60 * 60 * 1000; // 60 days
    ...
    <div className="fade-in">
      ...
      >
        ...
        <button
          ...
          onClick={() => exportToCSV(customers, 'customers_export')}
      ...
      >
        <div className="stat-card pink">
          ...
          <div className="stat-value" style={{ fontSize: isMobile ? '1.3rem' : undefined }}>
            {vipCount}
          </div>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <div>
                    ...
                    {c.phone && (
                      <div style={{ fontSize: '0.8rem', color: 'var(--text3)' }}>📞 {c.phone}</div>
                    )}
                  </div>
                </div>
                ...
                {filtered.map((c) => (
                  <tr key={c.id}>
                    ...
                    <td>
                      ...
                      {c.instagram && (
                        <div style={{ fontSize: '0.8rem', color: '#E1306C' }}>@{c.instagram}</div>
                      )}
                    </td>
                    <td>
      ...
      <AnimatedDemo moduleId="customers" title="Manage Customers" scenes={customersDemoScenes} />
    </div>
  );
}
