...
export async function requestStoragePermission() {
  if (!isNative()) {
    console.log('[Permissions] Not native — storage permission auto-granted');
    return 'granted';
  }
  ...
  try {
    ...
    console.log('[Permissions] Storage check result:', status);

    if (status.publicStorage === 'granted') return 'granted';

    // Request if not yet granted
    const requested = await Filesystem.requestPermissions();
    console.log('[Permissions] Storage request result:', requested);
    return requested.publicStorage;
  } catch (e) {
    console.warn('[Permissions] Filesystem permission error:', e?.message || e);
...
export async function requestCameraPermission() {
  if (!isNative()) {
    console.log('[Permissions] Not native — camera permission auto-granted');
    return 'granted';
  }
  ...
  try {
    ...
    console.log('[Permissions] Camera check result:', status);
    ...
    console.log('[Permissions] Camera request result:', requested);

    const granted =
      requested.camera === 'granted' && requested.photos === 'granted';
    return granted ? 'granted' : 'denied';
  } catch (e) {
    console.warn('[Permissions] Camera permission error:', e?.message || e);
    return 'denied';
  }
}
...
export async function checkPermissionsOnStartup() {
  ...
  try {
    ...
    console.log('[Permissions] Startup status:', {
    ...
    console.warn('[Permissions] Startup check error:', e?.message || e);
  }
}
...
export async function withStoragePermission(action, onDenied) {
  ...
  console.warn('[Permissions] Storage permission denied:', status);
