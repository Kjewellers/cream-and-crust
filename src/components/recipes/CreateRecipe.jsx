...
const DEFAULT_STEPS = [
  ...
  {
    title: 'Sieve Dry Ingredients',
    desc: 'Sieve flour, cocoa powder, baking soda and salt.',
    timer: '',
  },
  {
...
export default function CreateRecipe({ onClose, existingRecipe }) {
  ...
  const [data, setData] = useState({
    ...
    yield: '',
    imageUrl: '',
    ingredients: [{ name: '', qty: '', unit: '', cost: '' }],
    steps: [{ title: '', desc: '', timer: '' }],
    sellPrice: '',
    packaging: 0,
    gas: 0,
    labor: 0,
  ...
  useEffect(() => {
    if (existingRecipe) {
      setData((prev) => ({
        ...
        ingredients:
          Array.isArray(existingRecipe.ingredients) && existingRecipe.ingredients.length
            ? existingRecipe.ingredients
            : DEFAULT_INGS,
        steps:
          Array.isArray(existingRecipe.steps) && existingRecipe.steps.length
  ...
  const platformAmt =
    (ingCost + data.packaging + data.gas + data.labor + data.other) * (data.platformFee / 100);
  ...
  const validIngredients = data.ingredients.filter(
    (i) => i.name?.trim() && Number(i.qty) > 0 && i.unit
  );
  const validSteps = data.steps.filter((st) => st.title?.trim() || st.desc?.trim());

  const updateIng = (i, f, v) => {
  ...
  const validateStep = (currentStep = step, publish = false) => {
    if (!data.name.trim()) {
      showToast('Add a recipe name first', 'error');
      setStep(1);
      return false;
    }
    if (currentStep >= 2 && validIngredients.length === 0) {
      showToast('Add at least one ingredient with quantity', 'error');
      setStep(2);
      return false;
    }
    if (currentStep >= 3 && validSteps.length === 0) {
      showToast('Add at least one recipe step', 'error');
      setStep(3);
      return false;
    }
    if (publish && (!data.sellPrice || Number(data.sellPrice) <= 0)) {
      showToast('Set a selling price before publishing', 'error');
      setStep(4);
      return false;
    }
    return true;
  ...
  const save = async (publish = false) => {
    if (!validateStep(publish ? 4 : 1, publish)) return;
    try {
      const final = {
        ...data,
        ingredients: validIngredients.length ? validIngredients : data.ingredients,
        steps: validSteps.length ? validSteps : data.steps,
        status: publish ? 'Published' : 'Draft',
        badge: isEditing ? data.badge : 'New',
      };
  ...
  return (
    ...
    >
              ...
              <div key={i} style={{ flex: 1 }}>
                <div
                  style={{
                    height: 4,
                    borderRadius: 2,
                    background: step > i ? 'var(--rv-pink)' : '#E5E7EB',
                    transition: 'all 0.3s',
                  }}
                />
                <div
                  style={{
                    fontSize: 10,
                    fontWeight: 700,
                    color: step === i + 1 ? 'var(--rv-pink)' : 'var(--rv-muted)',
                    marginTop: 4,
                    textAlign: 'center',
                  }}
                >
      ...
      <div className="rv-wizard-content">
        ...
        {step === 1 && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            >
              ...
              >
                {uploading ? (
                  ...
                  <>
                    <Camera size={28} color="var(--rv-pink)" />
                    <div
                      style={{
                        fontSize: 11,
                        color: 'var(--rv-pink)',
            ...
            <div style={row}>
              <label style={s}>Recipe Name</label>
              <input
                style={inp}
                value={data.name}
            ...
            <div style={row}>
              <label style={s}>Category</label>
              <select
                style={inp}
                value={data.category}
                onChange={(e) => setData({ ...data, category: e.target.value })}
              >
                {['Cakes', 'Cupcakes', 'Brownies', 'Desserts', 'Bread', 'Cookies'].map((c) => (
            ...
            >
              <div>
                <label style={s}>Difficulty</label>
                <select
                  style={inp}
              ...
              <div>
                <label style={s}>Yield</label>
                <select
                  style={inp}
                  value={data.yield}
                  onChange={(e) => setData({ ...data, yield: e.target.value })}
                >
                  {YIELD_OPTIONS.map((y) => (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
              ...
              <div>
                <label style={s}>Baking Time</label>
                <input
                  style={inp}
                  value={data.bakeTime}
                  onChange={(e) => setData({ ...data, bakeTime: e.target.value })}
                  placeholder="45 mins"
                />
              </div>
              <div>
                <label style={s}>Cooling Time</label>
                <input
                  style={inp}
                  value={data.coolTime}
                  onChange={(e) => setData({ ...data, coolTime: e.target.value })}
                  placeholder="2 hrs"
                />
              </div>
            </div>
            ...
            <div style={{ ...row, marginTop: 18 }}>
              <label style={s}>Season (Optional)</label>
              <select
                style={inp}
                value={data.season}
        ...
        {step === 2 && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            {data.ingredients.map((ing, i) => (
              ...
              >
                <input
                  style={{ ...inp, padding: '9px 8px', fontSize: 13 }}
                  value={ing.name}
                  onChange={(e) => updateIng(i, 'name', e.target.value)}
                  placeholder="Flour"
                />
                <input
                  style={{ ...inp, padding: '9px 6px', fontSize: 13 }}
                  type="number"
                  value={ing.qty}
                  onChange={(e) => updateIng(i, 'qty', e.target.value)}
                />
                <select
                  style={{ ...inp, padding: '9px 4px', fontSize: 13 }}
                  value={ing.unit}
                  onChange={(e) => updateIng(i, 'unit', e.target.value)}
                >
                  {['g', 'kg', 'ml', 'L', 'pcs', 'tsp', 'tbsp'].map((u) => (
            ...
            <button
              onClick={() =>
                setData({
                  ...data,
                  ingredients: [...data.ingredients, { name: '', qty: '', unit: 'g', cost: 0 }],
                })
              }
              style={{
                color: 'var(--rv-pink)',
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            {data.steps.map((st, i) => (
              ...
              >
                ...
                >
                  <input
                    style={{ ...inp, marginBottom: 8, fontWeight: 600 }}
                    value={st.title}
                    onChange={(e) => {
            ...
            <button
              onClick={() =>
                setData({ ...data, steps: [...data.steps, { title: '', desc: '', timer: '' }] })
              }
              style={{
                color: 'var(--rv-pink)',
                background: 'var(--rv-pink-light)',
                border: '1px dashed var(--rv-pink)',
                borderRadius: 10,
                fontWeight: 700,
                fontSize: 14,
                cursor: 'pointer',
        ...
        {step === 4 && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            >
              ...
              <div style={{ marginTop: 14 }}>
                ...
                >
                  ...
                  <div style={{ width: 70, textAlign: 'right', color: 'var(--rv-pink)' }}>
                    Scaled
                  </div>
                </div>
                {data.ingredients.slice(0, 6).map((ing, i) => (
                  ...
                  >
                    ...
                    <div style={{ width: 70, textAlign: 'right', color: 'var(--rv-muted)' }}>
                      {ing.qty} {ing.unit}
                    </div>
                    <div style={{ width: 70, textAlign: 'right', fontWeight: 700 }}>
                      {(Number(ing.qty) * scaleFactor).toFixed(0)} {ing.unit}
                    </div>
                  </div>
                ))}
              </div>
            ...
            >
              <div
                style={{ fontWeight: 800, fontSize: 16, color: 'var(--rv-pink)', marginBottom: 2 }}
              >
                Total Cost ({data.yield})
              </div>
              <div
                style={{ fontSize: 26, fontWeight: 900, color: 'var(--rv-dark)', marginBottom: 14 }}
              >
                ₹{totalCost.toFixed(2)}
              </div>
            ...
            >
              <div>
                <div style={{ fontSize: 12, color: 'var(--rv-muted)', fontWeight: 600 }}>
                  Profit
                </div>
                <div
                  style={{
      ...
      <div className="rv-bottom-action" style={{ display: 'flex', gap: 12 }}>
        {step < 4 ? (
          <button className="rv-btn-primary" style={{ width: '100%' }} onClick={nextStep}>
          ...
          <>
            <button
              ...
              style={{ flex: 1, background: '#F3F4F6', color: 'var(--rv-dark)', boxShadow: 'none' }}
              onClick={() => save(false)}
            >
              Save Draft
            </button>
            <button className="rv-btn-primary" style={{ flex: 2 }} onClick={() => save(true)}>
              Save Recipe 🎉
            </button>
          </>
        )}
