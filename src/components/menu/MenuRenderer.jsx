...
import DoodleMenuRenderer from './DoodleMenuRenderer';
import EditorialMenuRenderer from './EditorialMenuRenderer';
import MidnightMenuRenderer from './MidnightMenuRenderer';
import GalleryMenuRenderer from './GalleryMenuRenderer';
import CinemaMenuRenderer from './CinemaMenuRenderer';
import MaisonMenuRenderer from './MaisonMenuRenderer';
import MaisonErrorBoundary from './MaisonErrorBoundary';
...
const CategoryPill = ({ category, active, onClick }) => (
  <button
    type="button"
    className={`menu-category-pill ${active ? 'active' : ''}`}
    onClick={onClick}
  >
...
export default function MenuRenderer({ business, settings, products, preview = false }) {
  ...
  // Theme-template branching. The doodle template is a fully separate
  // renderer with its own aesthetic — drawn doodles, polaroid cards,
  // animated reveals, motion-website feel. The default 'classic' path
  // below is unchanged for every existing user.
  if (data?.theme?.template === 'doodle') {
    return (
      <DoodleMenuRenderer
        business={business}
        settings={settings}
  ...
  // Editorial Magazine template — Vogue/Kinfolk inspired. Cream paper,
  // oversized Playfair serif, slow parallax hero, image zoom on scroll,
  // numbered table of contents, drop-cap editor's note.
  if (data?.theme?.template === 'editorial') {
    return (
      <EditorialMenuRenderer
        business={business}
        settings={settings}
        products={products}
        preview={preview}
  ...
  // Midnight Velvet template — dark luxury. Deep maroon + gold leaf,
  // candlelit glow, spotlight reveals, slow camera pan on hero,
  // gold-shimmer hover on cards.
  if (data?.theme?.template === 'midnight') {
    return (
      <MidnightMenuRenderer
        business={business}
        settings={settings}
        products={products}
        preview={preview}
  ...
  // Pastel Gallery template — museum exhibition. Custom cursor, mouse
  // spotlight, 3D tilt hero with floor reflection, falling-leaf card
  // entrances + idle bobbing, plaque hover flips, magnetic CTAs.
  if (data?.theme?.template === 'gallery') {
    return (
      <GalleryMenuRenderer
        business={business}
        settings={settings}
  ...
  // Cinema Pâtisserie — full cinematic theatre experience. Studio ident
  // intro, marquee with chase-light bulbs, director's note, trailer
  // reel, movie poster cards, scene markers, behind-the-scenes pull
  // quote, vintage ticket-stub CTA, rolling end credits.
  if (data?.theme?.template === 'cinema') {
    return (
      <CinemaMenuRenderer
        business={business}
        settings={settings}
  ...
  // Maison Atelier — couture pâtisserie. Sugar dust particles, flowing
  // gold ribbon, self-drawing monogram, sticky-scroll signature
  // showcase, glassmorphism vitrines, animated count-up stats, and a
  // closing reserve-the-door reveal.
  if (data?.theme?.template === 'maison') {
    return (
      <MaisonErrorBoundary>
        <MaisonMenuRenderer
          business={business}
    ...
    hero: () => (
      ...
      >
        ...
        <img
          ...
          src={data.heroImage || MENU_TEMPLATE_ASSETS.truffle}
          alt="Featured cake"
        />
      </motion.section>
    ),
    categories: () => (
      <section key="categories" className="menu-categories-section">
        ...
        <div className="menu-category-row">
          <CategoryPill
            category={{ name: 'All', image: MENU_TEMPLATE_ASSETS.truffle }}
      ...
      bestsellers.length > 0 && (
        <section key="bestsellers" className="menu-section">
          ...
          <motion.div
            ...
            className="menu-product-grid"
          >
            {bestsellers.map((product) => (
              <ProductCard key={product.id} product={product} onOrder={orderProduct} />
            ))}
          </motion.div>
        </section>
      ),
    ...
    footer: () => (
      <footer key="footer" className="menu-footer">
        <div>
          ...
          <small>
            © {new Date().getFullYear()} {data.bakeryName}. All Rights Reserved.
          </small>
        </div>
        <div className="menu-footer-contact">
          ...
          <p>
            <MapPin size={18} />
            <span>{data.deliveryLocations || data.city || 'Local delivery available'}</span>
          </p>
          <p>
            <Clock3 size={18} />
  ...
  return (
    <div
      ...
      style={{
        ...
        '--menu-button-radius':
          data.theme.buttonStyle === 'classic'
            ? '8px'
            : data.theme.buttonStyle === 'soft'
              ? '14px'
              : '999px',
        '--menu-density':
          data.theme.spacingDensity === 'compact'
            ? '0.82'
            : data.theme.spacingDensity === 'airy'
              ? '1.18'
              : '1',
      }}
    >
