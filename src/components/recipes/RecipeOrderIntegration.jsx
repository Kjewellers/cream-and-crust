  ...
  return (
    ...
    >
      ...
      <div className="rv-wizard-content">
        {/* Flow diagram */}
        <div style={{ background: 'var(--rv-pink-light)', borderRadius: 16, padding: '20px', marginBottom: 24, textAlign: 'center' }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--rv-muted)', marginBottom: 8 }}>WHEN AN ORDER IS PLACED</div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            <div style={{ background: 'var(--rv-pink)', color: '#fff', padding: '10px 18px', borderRadius: 20, fontWeight: 800, fontSize: 14 }}>
              Order Placed
            </div>
          ...
          {FLOW_STEPS.map((step, i) => (
            <motion.div
              ...
              style={{
                ...
                boxShadow: 'var(--rv-shadow-sm)', border: '1px solid var(--rv-border)'
              }}
            >
              <div style={{
        ...
        <div style={{ marginTop: 32, background: 'var(--rv-cream)', borderRadius: 16, padding: 20, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 14, color: 'var(--rv-muted)', lineHeight: 1.6 }}>
            Every order automatically triggers the complete recipe workflow — from scaling to checklist generation.
          </div>
        </div>
      </div>

      <div className="rv-bottom-action">
        <button className="rv-btn-primary" onClick={() => { onClose(); window.location.href = '/orders'; }}>
          View Orders →
        </button>
      </div>
    </motion.div>
