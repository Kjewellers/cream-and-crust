...
export default function RecipeDetail({
  ...
  // Guard against null/undefined recipe (can happen during AnimatePresence exit)
  ...
  // Defensive: filter out null/falsy ingredient entries that can come from
  // partial Firestore saves. This is the canonical source for the cost reducer
  // AND the inventory/scaling tabs, so it must never include null entries.
  const safeIngredients = (recipe.ingredients || []).filter((i) => i && typeof i === 'object');
  const safeSteps = (recipe.steps || []).filter((s) => s && typeof s === 'object');

  const ingCost = safeIngredients.reduce((sum, i) => sum + Number(i.cost || 0), 0);
  ...
  const allergens = [];
  const ingStr = JSON.stringify(safeIngredients).toLowerCase();
  if (ingStr.includes('nut') || ingStr.includes('almond') || ingStr.includes('peanut'))
    allergens.push('🥜 Nuts');
  if (ingStr.includes('milk') || ingStr.includes('cream') || ingStr.includes('butter'))
  ...
  return (
    ...
    >
      ...
      <div className="rv-details-hero">
        <img
          src={
            recipe.imageUrl || 'https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=800'
          }
          className="rv-details-img"
          alt=""
        ...
        <div className="rv-details-topbar">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="rv-circle-btn" onClick={() => onEdit && onEdit(recipe)}>
              <Edit2 size={18} />
            </button>
            <div style={{ position: 'relative' }}>
              <button className="rv-circle-btn" onClick={() => setMenuOpen((v) => !v)}>
      ...
      <div className="rv-details-content">
        ...
        >
          <div className="rv-details-title" style={{ marginBottom: 0, flex: 1, paddingRight: 8 }}>
            {recipe.name}
          </div>
          <div
            className={`rv-status-pill ${recipe.status === 'Published' ? 'published' : 'draft'}`}
          >
            {recipe.status || 'Draft'}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
          <span className="rv-tag" style={{ background: 'var(--rv-pink)', color: '#fff' }}>
            🎂 {recipe.category}
          </span>
          <span className="rv-tag">{recipe.yield}</span>
          {recipe.difficulty && <span className="rv-tag">{recipe.difficulty}</span>}
          {allergens.map((a) => (
            <span key={a} className="rv-tag" style={{ background: '#FEF3C7', color: '#D97706' }}>
              {a}
            </span>
          ))}
        </div>
        ...
        <div className="rv-tabs">
          {TABS.map(({ id, label, Icon }) => (
            <button
              key={id}
              className={`rv-tab ${tab === id ? 'active' : ''}`}
              onClick={() => setTab(id)}
              style={{
                display: 'flex',
                alignItems: 'center',
        ...
        {tab === 'Steps' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            {safeSteps.map((s, i) => (
              <div key={i} className="rv-step-card">
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div className="rv-step-text" style={{ color: 'var(--rv-muted)' }}>
                    {s.desc}
                  </div>
                  {s.timer && (
                    <div
            ...
            {!safeSteps.length && (
              <div style={{ padding: 24, textAlign: 'center', color: 'var(--rv-muted)' }}>
                No steps added yet.
              </div>
            )}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <button
                    ...
                    style={{
                      padding: '8px 12px',
                      borderRadius: 8,
                      border: '1.5px solid var(--rv-pink)',
                      background: 'var(--rv-pink-light)',
                      color: 'var(--rv-pink)',
                      fontWeight: 700,
                      fontSize: 14,
            ...
            {safeIngredients.map((ing, i) => (
              ...
              >
                ...
                <div style={{ width: 80, textAlign: 'right', color: 'var(--rv-muted)' }}>
                  {ing.qty} {ing.unit}
                </div>
                <div
                  style={{
                ...
                >
                  {(Number(ing.qty) * yieldMult).toFixed(1)} {ing.unit}
                </div>
              </div>
            ))}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            >
              ...
              >
                ...
                <span style={{ fontWeight: 900, fontSize: 20, color: 'var(--rv-dark)' }}>
                  ₹{simPrice}
                </span>
              </div>
              <input
                type="range"
                min={Math.max(0, Math.floor(totalCost / 50) * 50)}
                max={Math.max(1000, sellPrice * 2)}
                step="10"
                value={simPrice}
                onChange={(e) => setSimPrice(Number(e.target.value))}
                style={{ width: '100%', accentColor: 'var(--rv-pink)' }}
              />
              <div
                style={{
                  fontSize: 11,
              ...
              >
                Drag to simulate profit margins
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              >
                ...
                >
                  {currentSellPrice ? ((profit / currentSellPrice) * 100).toFixed(1) : 0}%
                </div>
              </div>
            </div>
          </motion.div>
        ...
        {tab === 'Notes' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            >
              <div style={{ fontWeight: 700, color: '#B45309', marginBottom: 10 }}>
                🔑 Kitchen Notes & Tips
              </div>
              <p
                style={{
              ...
              >
                {recipe.notes ||
                  "No notes added. Click 'Edit' to add kitchen secrets, storage tips, or supplier info."}
              </p>
            </div>
          </motion.div>
        )}
