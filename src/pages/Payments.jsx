  ...
  return (
    <div className="fade-in">
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
        ...
        {!isMobile && <button className="btn btn-outline"><Download size={18} /> Export Report</button>}
      </div>
      ...
      <div style={{
        ...
        <div className="stat-card orange">
          ...
          <div className="stat-value" style={{ fontSize: isMobile ? '1.2rem' : undefined }}>{formatCurrency(totalRevenue)}</div>
          <div className="stat-change" style={{ color: 'var(--text3)' }}>All orders</div>
        </div>
        <div className="stat-card green">
          <div className="stat-icon green"><CheckCircle size={18} /></div>
          <div className="stat-label">Collected</div>
          <div className="stat-value" style={{ fontSize: isMobile ? '1.2rem' : undefined }}>{formatCurrency(collected)}</div>
        </div>
        <div className="stat-card pink">
          <div className="stat-icon pink"><Clock size={18} /></div>
          <div className="stat-label">Pending</div>
          <div className="stat-value" style={{ color: 'var(--accent2)', fontSize: isMobile ? '1.2rem' : undefined }}>{formatCurrency(pending)}</div>
        </div>
        <div className="stat-card" style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
          <div className="stat-icon" style={{ background: 'rgba(61,139,106,0.12)' }}><DollarSign size={18} color="#3D8B6A" /></div>
          <div className="stat-label">Paid Orders</div>
          <div className="stat-value" style={{ fontSize: isMobile ? '1.2rem' : undefined }}>{paidCount}</div>
        </div>
      </div>
      ...
      <div className="card table-card">
        ...
        {isMobile ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: '0 4px 4px' }}>
            {payments.length === 0 && (
              <div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--text3)' }}>No payments yet</div>
            )}
            {payments.map(p => (
              <div key={p.id} style={{
                background: 'var(--bg)',
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontWeight: 700, fontSize: '0.95rem' }}>{p.id} · {p.customer}</span>
                  <span style={{
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4 }}>
                  <div>
                    <div style={{ fontSize: '0.7rem', color: 'var(--text3)', marginBottom: 2 }}>Total</div>
                    <div style={{ fontWeight: 600, fontSize: '0.9rem' }}>{formatCurrency(p.total)}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.7rem', color: 'var(--text3)', marginBottom: 2 }}>Collected</div>
                    <div style={{ fontWeight: 600, fontSize: '0.9rem', color: '#3D8B6A' }}>{formatCurrency(p.advance)}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.7rem', color: 'var(--text3)', marginBottom: 2 }}>Pending</div>
                    <div style={{ fontWeight: 600, fontSize: '0.9rem', color: p.pending > 0 ? 'var(--accent2)' : 'var(--text3)' }}>{formatCurrency(p.pending)}</div>
                  </div>
                </div>
              ...
              <tbody>
                {payments.map(p => (
                  <tr key={p.id}>
                    ...
                    <td style={{ color: p.pending > 0 ? 'var(--accent2)' : 'var(--text3)', fontWeight: 600 }}>
